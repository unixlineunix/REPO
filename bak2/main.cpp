...
static void train_parallel_fork(const std::vector<int>& data, int V, int H,
    ...
    for (int w = 0; w < num_workers; w++) {
        ...
        if (pid == 0) {
            ...
            NeuralLM model(vk_child, V, H, lr, batch_size, temperature, top_k);
...
static std::string collect_corpus(const std::string& datadir) {
    ...
    if (!trdat_files.empty()) {
        ...
        for (const auto& f : trdat_files) {
            ...
            if (!content.empty()) {
                std::cout << "Loaded corpus: " << f << " (" << content.size() << " chars)" << std::endl;
                result += content;
                result += '\n';
            }
        }
    ...
    try {
        ...
        if (parallel_workers > 1) {
            ...
            train_parallel_fork(data, tk.vocab_size, n_embd, lr, batch_size, temperature,
                                num_cores, parallel_workers, save_path, max_epochs, continue_training, top_k, save_every_epoch);
